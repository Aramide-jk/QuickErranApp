    ...
    <div className="bg-white rounded-xl p-4 lg:p-6 border shadow-sm hover:shadow-md transition-shadow">
      <div className="flex justify-between items-start mb-3">
        <div className="flex-1">
          ...
          <div className="space-y-1 text-sm">
            <p><span className="text-gray-500">Pickup:</span> {request.pickupAddress}</p>
            <p><span className="text-gray-500">Delivery:</span> {request.deliveryAddress}</p>
          </div>
        </div>
        
        <div className="text-right ml-4">
          <span className="text-2xl font-bold text-primary">₦{request.budget.toLocaleString()}</span>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between pt-3 border-t">
        <div className="flex items-center">
          ...
          <div>
            <p className="font-medium text-text-dark text-sm">{request.customer.name}</p>
            <div className="flex items-center">
              <Star className="w-3 h-3 text-yellow-500 mr-1" />
              <span className="text-sm text-gray-600">{request.customer.rating}</span>
            </div>
          </div>
        </div>
      ...
      {sidebarOpen && (
        <div className="lg:hidden fixed inset-0 bg-black bg-opacity-50 z-50" onClick={() => setSidebarOpen(false)}>
          <div className="w-80 bg-white h-full" onClick={(e) => e.stopPropagation()}>
            <DesktopSidebar user={user} currentPage="runner-dashboard" mobile onClose={() => setSidebarOpen(false)} />
          </div>
        </div>
      )}
          ...
          <div className="bg-primary text-white p-4 lg:p-6">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center">
                ...
                <div>
                  <h1 className="text-xl lg:text-2xl font-bold">Runner Dashboard</h1>
                  <p className="text-white text-opacity-90 text-sm">Welcome back, {user?.name?.split(' ')[0]}</p>
                </div>
              </div>
          ...
          <div className="p-4 lg:p-6">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
              <div className="bg-white rounded-xl p-4 border">
                ...
                <p className="text-2xl font-bold text-text-dark">₦{stats.todayEarnings.toLocaleString()}</p>
              ...
              <div className="bg-white rounded-xl p-4 border">
                ...
                <p className="text-2xl font-bold text-text-dark">{stats.completedToday}</p>
              ...
              <div className="bg-white rounded-xl p-4 border">
                ...
                <p className="text-2xl font-bold text-text-dark">{stats.rating}</p>
              ...
              <div className="bg-white rounded-xl p-4 border">
                ...
                <p className="text-2xl font-bold text-text-dark">₦{stats.totalEarnings.toLocaleString()}</p>
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-xl lg:text-2xl font-bold text-text-dark">Available Requests</h2>
              ...
              {isOnline ? (
                ...
                <div className="bg-white rounded-xl p-8 border text-center">
                  ...
                  <p className="text-gray-500 mb-4">Turn on your availability to see and accept requests</p>
                  <button
